//
//  KLKVStorage.h
//  KLCache <https://github.com/ibireme/KLCache>
//
...
/**
 KLKVStorageItem is used by `KLKVStorage` to store key-value pair and meta data.
 Typically, you should not use this class directly.
 */
@interface KLKVStorageItem : NSObject
...
/**
 Storage type, indicated where the `KLKVStorageItem.value` stored.
 ...
 * If you want to store large number of small datas (such as contacts cache), 
   use KLKVStorageTypeSQLite to get better performance.
 * If you want to store large files (such as image cache),
   use KLKVStorageTypeFile to get better performance.
 * You can use KLKVStorageTypeMixed and choice your storage type for each item.
 
 See <http://www.sqlite.org/intern-v-extern-blob.html> for more information.
 */
typedef NS_ENUM(NSUInteger, KLKVStorageType) {
    
    /// The `value` is stored as a file in file system.
    KLKVStorageTypeFile = 0,
    
    /// The `value` is stored in sqlite with blob type.
    KLKVStorageTypeSQLite = 1,
    
    /// The `value` is stored in file system or sqlite based on your choice.
    KLKVStorageTypeMixed = 2,
};



/**
 KLKVStorage is a key-value storage based on sqlite and file system.
 Typically, you should not use this class directly.
 
 @discussion The designated initializer for KLKVStorage is `initWithPath:type:`. 
 ...
 */
@interface KLKVStorage : NSObject
...
@property (nonatomic, readonly) KLKVStorageType type;  ///< The type of this storage.
 ...
 */
- (nullable instancetype)initWithPath:(NSString *)path type:(KLKVStorageType)type NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 If the `type` is KLKVStorageTypeFile, then the item.filename should not be empty.
 If the `type` is KLKVStorageTypeSQLite, then the item.filename will be ignored.
 It the `type` is KLKVStorageTypeMixed, then the item.value will be saved to file 
 system if the item.filename is not empty, otherwise it will be saved to sqlite.
 
 @param item  An item.
 @return Whether succeed.
 */
- (BOOL)saveItem:(KLKVStorageItem *)item;

/**
 Save an item or update the item with 'key' if it already exists.
 
 @discussion This method will save the key-value pair to sqlite. If the `type` is
 KLKVStorageTypeFile, then this method will failed.
...
/**
 ...
 @discussion
 If the `type` is KLKVStorageTypeFile, then the `filename` should not be empty.
 If the `type` is KLKVStorageTypeSQLite, then the `filename` will be ignored.
 It the `type` is KLKVStorageTypeMixed, then the `value` will be saved to file
 ...
 */
- (nullable KLKVStorageItem *)getItemForKey:(NSString *)key;
 ...
 */
- (nullable KLKVStorageItem *)getItemInfoForKey:(NSString *)key;
...
/**
 ...
 @param keys  An array of specified keys.
 @return An array of `KLKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<KLKVStorageItem *> *)getItemForKeys:(NSArray<NSString *> *)keys;
...
/**
 ...
 @return An array of `KLKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<KLKVStorageItem *> *)getItemInfoForKeys:(NSArray<NSString *> *)keys;
