...
static void _KLDiskCacheInitGlobal() {
...
static KLDiskCache *_KLDiskCacheGetGlobal(NSString *path) {
    if (path.length == 0) return nil;
    _KLDiskCacheInitGlobal();
...
static void _KLDiskCacheSetGlobal(KLDiskCache *cache) {
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"KLDiskCache init error" reason:@"KLDiskCache must be initialized with a path. Use 'initWithPath:' or 'initWithPath:inlineThreshold:' instead." userInfo:nil];
...
- (instancetype)initWithPath:(NSString *)path
    ...
    KLDiskCache *globalCache = _KLDiskCacheGetGlobal(path);
    if (globalCache) return globalCache;
    
    KLKVStorageType type;
    if (threshold == 0) {
        type = KLKVStorageTypeFile;
    } else if (threshold == NSUIntegerMax) {
        type = KLKVStorageTypeSQLite;
    } else {
        type = KLKVStorageTypeMixed;
    }
    
    KLKVStorage *kv = [[KLKVStorage alloc] initWithPath:path type:type];
    if (!kv) return nil;
    
    _kv = kv;
    _path = path;
    _lock = dispatch_semaphore_create(1);
    _queue = dispatch_queue_create("com.Kalan.cache.disk", DISPATCH_QUEUE_CONCURRENT);
...
- (id<NSCoding>)objectForKey:(NSString *)key {
    if (!key) return nil;
    Lock();
    KLKVStorageItem *item = [_kv getItemForKey:key];
    Unlock();
    ...
    if (object && item.extendedData) {
        [KLDiskCache setExtendedData:item.extendedData toObject:object];
    }
    return object;
}
...
- (void)setObject:(id<NSCoding>)object forKey:(NSString *)key {
    ...
    NSData *extendedData = [KLDiskCache getExtendedDataFromObject:object];
    NSData *value = nil;
    ...
    if (!value) return;
    NSString *filename = nil;
    if (_kv.type != KLKVStorageTypeSQLite) {
